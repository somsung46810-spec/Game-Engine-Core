...
AI Generation Lab (/ai-lab) — Prompt input with pluggable provider selector (GPT-4o, Claude, Gemini, Custom), Mode Picker (code/shader/scene/material/level), streaming output in a terminal-style block, generation history Pipeline — Visual flowchart of the full architectural grid (Pipeline → Sandbox Zones → Asset Registry → Safety Bounds → Safe Render → Active Viewport), kanban sprint board, workflow import/export Backend: Express API with PostgreSQL, all route groups, modular AI provider registry, physics-audio sync, 4K resolution support — seeded with 3 projects, 10 assets, 2 scenes, 15 performance logs
